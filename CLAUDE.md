...
**스타일 화면이 보여줄 값의 우선순위** — 코스 편집에서 열었을 때:

1. **편집 중인 경로에 적용된 스타일**(`RouteEditSession.appliedOption`)
2. 없으면 유저 프로필(`GET /user/{id}/riding-profile`)
...
1번에서 자연스럽게 반영된다. 마이페이지에서 연 화면은 언제나 유저 프로필이다.
...
- [x] **라이딩 스타일을 경로 요청에 반영 (TDD)** — 테스트 309개, 스킵 0
