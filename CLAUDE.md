...
`navigationManager.holdsRidingEditor`로 스택을 확인한 뒤에만 `finishEditSession()`을 부른다.
뒤로가기 버튼과 `endRiding`에도 직접 걸어 두었지만, **시스템 스와이프 백처럼
버튼을 거치지 않는 경로**가 있어 스택 판정이 최종 안전망이다.
`finishEditSession()`은 멱등이라 여러 곳에서 불려도 무해하다.
...
- [x] **라이딩 스타일을 경로 요청에 반영 (TDD)** — 테스트 325개, 스킵 0
