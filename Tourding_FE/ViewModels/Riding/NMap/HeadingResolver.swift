...
enum HeadingResolver {
    ...
    /// 실기기에서는 그 값으로도 마커가 왼쪽으로 치우쳐 보여 -3.5, 다시 +16.5로 조정했다.
    /// **주의: 오프셋을 20도씩 옮겨도 화면 변화가 뚜렷하지 않다는 보고가 있다.**
    /// 값이 실제로 화면에 반영되는지부터 확인이 필요하다 (아래 참고).
    /// **렌더된 아이콘의 실제 방향이 SVG 좌표 계산과 다르다** — 어디서 어긋나는지는 미규명.
    ///
    /// 이 값이 0에 가깝다는 건 보정이 거의 불필요하다는 뜻이다.
    /// 에셋 화살표를 정북으로 맞추면 이 상수를 지울 수 있다 (별건).
    static let markerIconOffset: CLLocationDirection = 16.5
