...
// MARK: - 바텀 시트 위치 열거형
enum RecommendBottomSheetPosition: CaseIterable {
    case small
    case medium
    case large
...
// MARK: - 커스텀 바텀 시트
struct RecommendBottomSheet<Content: View>: View {
    ...
    @Binding private var currentPosition: RecommendBottomSheetPosition
    ...
    init(
        ...
        currentPosition: Binding<RecommendBottomSheetPosition>,
        isRiding: Bool = false,
        ...
        GeometryReader { geometry in
            ZStack(alignment: .bottom) {
                ...
                VStack(spacing: 0) {
                    ...
                    // 컨텐츠
                    content
                        .frame(height: currentPosition.height(screenHeight: screenHeight))
    ...
    private func dragGesture(geometry: GeometryProxy) -> some Gesture {
        DragGesture()
            .onChanged { value in
                ...
                offset = max(0, min(screenHeight - RecommendBottomSheetPosition.small.height(screenHeight: screenHeight), newOffset))
            }
            .onEnded { value in
                isDragging = false
    ...
    // MARK: - Snap Logic
    private func determineTargetPosition(translation: CGFloat, velocity: CGFloat, currentPosition: RecommendBottomSheetPosition) -> RecommendBottomSheetPosition {
        let positions = RecommendBottomSheetPosition.allCases
    ...
    // MARK: - Animation
    private func animateToPosition(_ position: RecommendBottomSheetPosition) {
